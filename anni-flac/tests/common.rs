...
/// `1s` generated by:
/// ```bash
/// ffmpeg -f lavfi -i "sine=frequency=1000:duration=1" 1s.flac
/// ```
///
/// `1s-full` has seekpoints and cover:
/// ```bash
/// metaflac --add-seekpoint=1s 1s-full.flac
/// metaflac --import-picture-from=1s-cover.png 1s-full.flac
/// ```
///
/// and the metadata:
/// ```text
...
/// ```
#[test]
fn test_audio_file_exist() {
    assert!(std::path::Path::new("../assets/1s.flac").exists());
    assert!(std::path::Path::new("../assets/1s-full.flac").exists());
}

/// Cover generated by:
///
/// ```bash
/// ffmpeg -f lavfi -i color=white:640x480:d=3,format=rgb24 -frames:v 1 1s-cover.png
/// ```
#[test]
fn test_cover_file_exist() {
    let exist = std::path::Path::new("../assets/1s-cover.png").exists();
    assert!(exist);
}

pub fn parse_1s_audio() -> FlacHeader {
    FlacHeader::from_file("../assets/1s.flac").unwrap()
}

pub fn parse_full_1s_audio() -> FlacHeader {
    FlacHeader::from_file("../assets/1s-full.flac").unwrap()
}
    ...
    for (i, block) in stream.blocks.iter().enumerate().skip(1) {
        ...
        match &block.data {
            ...
            MetadataBlockData::Picture(picture) => {
                ...
                let mut file =
                    File::open("../assets/1s-cover.png").expect("Failed to open cover file.");
                let mut data = Vec::new();
                file.read_to_end(&mut data)
                    .expect("Failed to read test cover.");
...
fn test_blocks_format() {
    let mut stream = parse_full_1s_audio();
