...
impl FileSystemProvider for LocalFileSystemProvider {
    async fn children(
        &self,
        path: &PathBuf,
    ) -> crate::Result<Pin<Box<dyn Stream<Item = FileEntry> + Send>>> {
        Ok(Box::pin(stream::iter(std::fs::read_dir(path)?.filter_map(
            |entry| {
                let entry = entry.ok()?;
                let path = entry.path();
    ...
    async fn get_file_entry_by_prefix(
        &self,
        parent: &PathBuf,
        prefix: &str,
        ...
        loop {
            match dir.next_entry().await? {
                Some(entry) if entry.file_name().to_string_lossy().starts_with(prefix) => {
                    return Ok(FileEntry {
